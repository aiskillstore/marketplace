...
```bash
curl -X GET "${MAXHUB_BASE_URL}/api/v1/kuaishou/web/fetch_kuaishou_hot_list_v1" \
  -H "x-api-key: $MAXHUB_API_KEY"
```
...
```bash
curl -X GET "${MAXHUB_BASE_URL}/api/v1/kuaishou/web/fetch_one_video?share_text=https://example.com/share" \
  -H "x-api-key: $MAXHUB_API_KEY"
```
