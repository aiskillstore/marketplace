...
2. 按门禁要求：**在你即将动手做事或回答用户的前一刻**（写交付物 / 施工确认 / 产线命令 / 宣布交付，取最早），才读取 `DISCIPLINE.md`（五阶段流程全文在此文件，**不在** `SKILL.md`）；在那之前不要提前读取它。创意类可有一轮不读 DISCIPLINE 的方向构想；陌生专名仍先问/搜。随后按 `DISCIPLINE.md` 的五阶段执行——规则文件**按阶段**读取：阶段2 读
