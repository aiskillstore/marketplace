# Radiology Report Template

## Patient Information

**Patient Name:** [Last Name, First Name]  
**MRN:** [Medical Record Number]  
**Date of Birth:** [MM/DD/YYYY]  
**Age:** [Years]  
**Gender:** [Male/Female]  
**Date of Examination:** [MM/DD/YYYY]  
**Time of Examination:** [HH:MM]  
**Accession Number:** [Number]

**Referring Physician:** [Name]  
**Ordering Department:** [Department/Unit]

---

## Examination

**Examination Type:** [CT/MRI/X-ray/Ultrasound/PET/Nuclear Medicine Scan]  
**Body Part:** [Anatomic Region - e.g., Chest, Abdomen and Pelvis, Brain]  
**Contrast:** [Yes - IV/Oral/Both | No]  
**Laterality:** [Right/Left/Bilateral (if applicable)]

---

## Clinical Indication

[Reason for exam, relevant clinical history, specific questions to be answered]

Example: "Rule out pulmonary embolism in a patient with acute dyspnea and chest pain. Status post recent surgery."

---

## Comparison

**Prior Examinations:**  
[Modality] of the [Body Part] dated [Date]: [Available for comparison / Not available for comparison]

Example: "Non-contrast CT chest from 6 months ago (January 15, 2023) is available for comparison."

Or: "No prior imaging is available for comparison."

---

## Technique

[Detailed description of imaging parameters and protocols]

**CT Examination:**
```
Multidetector CT of the [body region] was performed [without/with] intravenous 
contrast. [Volume] mL of [iodinated contrast agent name] was administered 
intravenously. Images were acquired in the [arterial/venous/delayed] phase(s).
Multiplanar reconstructions were performed.

Technical quality: [Adequate / Limited by motion artifact / Limited by patient body habitus]
Radiation dose (DLP): [mGy-cm]
// Technical quality: [Good / Limited by motion artifact / Limited by patient body habitus]
// Radiation dose (DLP): [mGy-cm]
```

**MRI Examination:**
```
MRI of the [body region] was performed [without/with] intravenous contrast
using the following sequences: [list sequences - T1, T2, FLAIR, DWI, etc.]
[Volume] mL of [gadolinium-based contrast agent] was administered intravenously.
Multiplanar imaging was obtained.

Technical quality: [Adequate / Limited by motion artifact]
// Technical quality: [Good / Limited by motion artifact]
```

**X-ray Examination:**
```
[Number] views of the [body part] were obtained: [AP/PA/Lateral/Oblique]
Technical quality: [Adequate penetration and positioning / Limited by...]
// Technical quality: [Adequate penetration and positioning / Limited by...]
```

**Ultrasound Examination:**
```
Real-time ultrasound examination of the [body part] was performed using 
[linear/curved] array transducer.
Technical quality: [Adequate / Limited by bowel gas / Limited by body habitus]
// Technical quality: [Good / Limited by bowel gas / Limited by patient body habitus]
```

---

## Findings

[Systematic, comprehensive description of findings organized by anatomic region or organ system]

### [Region/Organ 1]

[Detailed findings - size, density/signal intensity, enhancement pattern, abnormalities]

**Normal Description:** "[Organ] is normal in size, morphology, and [attenuation/signal intensity]. No focal lesions identified."

**Abnormal Description:** "[Description of abnormality and measurements]"

Example:
```
Lungs:
- Bilateral ground-glass opacities are present, predominant in the lower lobes.
- Right lower lobe consolidation measuring 4.5 x 3.2 cm with air bronchograms.
- No pleural effusion or pneumothorax.
- Airways are patent bilaterally.
// Lungs:
// - Bilateral ground-glass opacities are present, predominant in the lower lobes.
// - Right lower lobe consolidation measuring 4.5 x 3.2 cm with air bronchograms.
// - No pleural effusion or pneumothorax.
// - Airways are patent bilaterally.
```

### [Region/Organ 2]

[Findings]

### [Other Applicable Regions]

**Chest CT:**
- Lungs
- Airways
- Pleura
- Mediastinum and hila
- Heart and great vessels
- Chest wall
- Upper abdomen (if included)
- Bones

**Abdomen/Pelvis CT:**
- Liver
- Gallbladder
- Spleen
- Pancreas
- Kidneys and adrenals
- Gastrointestinal tract
- Peritoneum and mesentery
- Retroperitoneum
- Bladder
- Pelvic organs
- Vasculature
- Lymph nodes
- Bones
- Soft tissues

**Brain MRI:**
- Brain parenchyma
- Ventricles and cisterns
- Extra-axial spaces
- Vascular structures
- Orbits (if included)
- Skull base and calvarium

### Measurements (if applicable)

| Structure | Measurement | Normal Range |
|-----------|-------------|--------------|
| [Lesion/Mass] | [Size in cm, 3D] | - |
| [Organ] | [Size] | [Normal Size] |

---

## Impression

[Brief summary of key findings and their clinical interpretation]

**Numbered list ordered by clinical importance:**

1. **[Most significant finding]** - [Diagnosis or differential diagnosis, clinical significance]
   - [Additional details, comparison with priors (if applicable)]
   - [Recommendations (if any)]

2. **[Second finding]** - [Interpretation]

3. **[Other findings]**

**Alternative format for normal studies:**
```
No acute intrathoracic abnormality.
Specifically, no evidence of pulmonary embolism.
// No acute intrathoracic abnormality.
// Specifically, no evidence of pulmonary embolism.
```

**Recommendations (if applicable):**
- [Further imaging, follow-up intervals, clinical correlation, biopsy, etc.]
- [Follow-up schedule]

Example:
```
Recommend follow-up CT in 3 months to assess for interval change.
Clinical correlation with laboratory values recommended.
Consider PET/CT for further characterization if clinically indicated.
// Recommend follow-up CT in 3 months to assess for interval change.
// Clinical correlation with laboratory values recommended.
// Consider PET/CT for further characterization if clinically indicated.
```

---

## Critical Results Communication

[If a critical/urgent finding is identified]

**Critical Finding:** [Description]

**Communicated to:** [Name and title of recipient]  
**Date/Time:** [MM/DD/YYYY at HH:MM]  
**Method:** [Telephone / Pager / In person]  
**Read-back confirmed:** [Yes]

---

## Structured Reporting (if applicable)

### Lung Nodules (Lung-RADS):
**Category:** [Lung-RADS 0/1/2/3/4A/4B/4X]  
**Recommendation:** [Follow Lung-RADS guidelines]

### Breast Imaging (BI-RADS):
**Category:** [BI-RADS 0/1/2/3/4/5/6]  
**Recommendation:** [Follow BI-RADS guidelines]

### Liver Lesions (LI-RADS):
**Category:** [LI-RADS 1/2/3/4/5/M/TIV]  
**Features:** [Arterial phase hyperenhancement, washout, capsule, size, growth]

### Prostate (PI-RADS):
**Score:** [PI-RADS 1/2/3/4/5]  
**Location:** [Peripheral zone / Transition zone]

---

## Signature

**Interpreting Physician:**  
[Radiologist Name, MD]  
[Qualifications]  
[NPI Number (if required)]

**Electronic Signature:** [Date and Time]

**Dictated Time:** [Date and Time]  
**Transcribed Time:** [Date and Time]  
**Signed Time:** [Date and Time]

---

## Template Notes

### General Principles

**Systematicity:**
- Use a consistent order (head-to-toe, outside-in)
- Do not skip relevant areas even if findings are normal
- Include pertinent negative findings

**Specificity:**
- Provide measurements (masses should have 3D sizes)
- Describe locations precisely
- Use standard terminology (RadLex)
- Quantify whenever possible

**Clarity:**
- Avoid ambiguous language
- Ensure the Impression can stand alone
- Answer clinical questions directly
- State what "is" present, not just what "is not"

**Communication:**
- Critical findings require immediate verbal notification
- Document communication details
- Provide specific recommendations
- Suggest next steps when appropriate

### Measurement Guidelines

**Lesions/Masses:**
- Three dimensions: [Length x Width x Height in cm]
- Use consistent measurement methods during follow-up

**Lymph Nodes:**
- Short-axis diameter (in cm)
- Note morphology (round vs. oval)

**Organ Size:**
- Use established normal ranges
- Adjust for age and gender

### Comparison Statements

**Improved:**
"The right upper lobe mass has decreased in size from 3.5 cm to 2.1 cm."

**Stable:**
"The 8 mm left lower lobe nodule is unchanged, stable for 2 years."

**Worsened:**
"Bilateral pleural effusions have increased, now moderate on the right."

**New Finding:**
"New 1.5 cm right adrenal nodule, not seen on prior CT."

### Differential Diagnosis Terminology

**Definite:** "Consistent with..."  
**Probable:** "Most likely represents..." or "Favors..."  
**Possible:** "Suggestive of..." or "Differential diagnosis includes..."  
**Uncertain:** "Cannot exclude..." or "Consider..."

### Recommendations

**Imaging Follow-up:**
- Specify modality, timing, and focus of evaluation
- "Recommend follow-up CT chest in 6-12 months to assess for stability"

**Further Characterization:**
- "Consider MRI for further characterization"
- "Ultrasound correlation is recommended"

**Clinical Correlation:**
- "Recommend clinical correlation with tumor markers"
- "Please correlate with patient symptoms and physical examination"

**Biopsy/Intervention:**
- "Consider biopsy for definitive diagnosis"
- "Image-guided biopsy can be performed if clinically indicated"