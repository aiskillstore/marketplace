...
```bash
curl -X GET "${MAXHUB_BASE_URL}/api/v1/toutiao/web/get_article_info?aweme_id=123456" \
  -H "x-api-key: $MAXHUB_API_KEY"
```
...
```bash
curl -X GET "${MAXHUB_BASE_URL}/api/v1/toutiao/app/get_user_info?user_id=123456" \
  -H "x-api-key: $MAXHUB_API_KEY"
```
